import { Descartes, User } from "../db";

class descartesService{
    //게시글 작성
    static async addPost({ userId, title, content }){
        const author = await User.findById({ userId });
        const newPost = { author, title, content };
        const creatednewPost = await Descartes.create({ newPost });
    ...
    static async getPostInfo({ postId }){
        const post = await Descartes.findByPostId({ postId });
    ...
    static async getPostList(){
        const posts = await Descartes.findAll();
        return posts;
    }

    //게시글 수정
    static async setPost({ userId, postId, toUpdate }){
        let post = await Descartes.findByPostId({ postId });
        ...
        post = await Descartes.update({ postId, newValues });
        return post;
    }
    ...
    static async deletePost({ userId, postId }){
        ...
        const res = await Descartes.delete({ postId });
        return { status: "success" };
    }

}

export { descartesService };
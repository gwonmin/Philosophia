import { Aristotle, User } from "../db";

class aristotleService{
    //게시글 작성
    static async addPost({ userId, title, content }){
        const author = await User.findById({ userId });
        const newPost = { author, title, content };
        const creatednewPost = await Aristotle.create({ newPost });
    ...
    static async getPostInfo({ postId }){
        const post = await Aristotle.findByPostId({ postId });
    ...
    static async getPostList(){
        const posts = await Aristotle.findAll();
        return posts;
    }

    //게시글 수정
    static async setPost({ userId, postId, toUpdate }){
        let post = await Aristotle.findByPostId({ postId });
        ...
        post = await Aristotle.update({ postId, newValues });
        return post;
    }
    ...
    static async deletePost({ userId, postId }){
        ...
        const res = await Aristotle.delete({ postId });
        return { status: "success" };
    }

}

export { aristotleService };
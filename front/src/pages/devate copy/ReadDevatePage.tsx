...
export default function ReadFreePage({
  ...
  const deleteHandler = async () => {
    if (devateId) {
      try {
        await Api.delete({ endpoint: "free", params: devateId })
  ...
  const agreeHandler = async () => {
    ...
    try {
      const res = await Api.put({ endpoint: `free/${devateId}/stance`, data: { stance: "1" } })
  ...
  const disagreeHandler = async () => {
    ...
    try {
      const res = await Api.put({ endpoint: `free/${devateId}/stance`, data: { stance: "0" } })
      setSomethingWasChanged(!somethingWasChanged)
